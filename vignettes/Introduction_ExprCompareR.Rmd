 ...
 **Image 1:** Depicts the functionality of functions `compute_correlation`, `correlation_genes_tissues`, `detect_outliers`, and `cancer_protein`. <br>
 
![](./images/ExprCompareR_Overview.png)
...
The function `compute_correlation` serves as a wrapper to compute and visualize the Spearman correlation between RNA and protein expression for a user's gene list **OR** tissue list of interest and plots the results.

<br>

`compute_correlation` performs a number of checks, to prevent the user from providing the wrong input type. The correct input type for `compute_correlation` is either a list of gene names (>=5 in length) **OR** a list of tissue names (>=5 in length). Valid gene symbols for the {gene_NAMES} argument can be accessed from {gene_symbols_list}. Valid tissue names as for the {tissue_NAMES} argument can be accessed from
{tissue_map$protein_tissue}. To get more information about the datasets mentioned above, the help documentation will helpful to explore: 
...
# Explore the valid gene names
head(gene_symbols_list)
length(gene_symbols_list)

# Explore the valid tissue names
...
These are the following conditions which will result in stopping execution and an error being thrown:

1. The given gene names do not exist in {gene_symbols_list} or the given tissue names do not exist in {tissue_map$protein_tissue}.
2. If both `gene_NAMES` and `tissue_NAMES` are NULL.
3. If one list is provided but has fewer than 5 elements.
4. If both lists are provided with 5 >= elements each.
5. If duplicates exist in either list.

The following are examples of an error being thrown, and execution being stopped in `compute_correlation`:


```{r test_cc, eval=FALSE}

# Example 1: Test if provided list has less than 5 elements
compute_correlation(tissue_NAMES = c("liver", "lung", "spleen"))
"Need at least five tissue names."

# Example 2: Test if provided list has duplicate gene names
...
**Demonstration of function `compute_correlation`:**
<br>
<br>
...
This function serves as a wrapper to compute and visualize the Spearman correlation between RNA and protein expression for a user's gene list **AND** tissue list of interest and plots the results.
...
`correlation_genes_tissues` performs a number of checks, to prevent the user from providing the wrong input type. The correct input type for `correlation_genes_tissues` is a list of gene names(>=5 in length) **AND** a list of tissue names(>=5 in length) **AND** a choice of plot. Valid gene symbols for the {gene_NAMES} argument can be accessed from {gene_symbols_list}. Valid tissue names as for the {tissue_NAMES} argument can be accessed from
{tissue_map$protein_tissue}.Valid plot choices can only be `"per_gene"` to visualize correlation for each gene, or `"per_tissue"` to visualize correlation for each tissue.
...
1. The given gene names do not exist in {gene_symbols_list} or the given tissue names do not exist in {tissue_map$protein_tissue} or the plot_choice is not `"per_gene"` or `"per_tissue"`. 
2. If both `gene_NAMES` and `tissue_NAMES` are NULL.
3. If both lists are provided but have fewer than 5 elements each.
4. If duplicates exist in either list.

The following are examples of an error being thrown, and execution being stopped in `correlation_genes_tissues`:

```{r test_c_g_t, eval=FALSE}

# Example 1: Test if only one list if provided
valid_tissues <- c("lung", "heart", "liver", "brain", "kidney")
correlation_genes_tissues(NULL, valid_tissues, plot_choice = "per_gene")
"This function requires both gene_NAMES and tissue_NAMES. For only one list, use compute_correlation()."

# Example 2: Test if invalid plot choice is provided
valid_genes <- c("BRCA1", "TP53", "BRCA1", "MYC", "EGFR")
correlation_genes_tissues(valid_genes, valid_tissues, plot_choice = "invalid")
"Choice of variable plot_choice is invalid. Please choose between per_gene and per_tissue"
...
**Demonstration of function `correlation_genes_tissues`:**
<br>
<br>
...
The function, given a tissue of interest, compares the RNA expression and protein expression of all genes present in the given tissue, and identifies genes which have a large difference in their RNA and protein expression. These genes are identified as outlier genes, and are most likely candidates for post-transcriptional modifications. This function returns the list of outlier genes as well as a plot highlighting the outlier genes.
...
ExprCompareR's `detect_outliers` function depends on user input i.e. their choice of the tissue of interest. 
`detect_outliers` performs checks, to prevent the user from providing the wrong input type. The correct input type for `detect_outliers` is  valid character string specifying the tissue of interest to analyze.Valid tissue names are listed in tissue_map$protein_tissue. If an invalid tissue name is provided,
the function will stop execution and throw an error.

To get more information about the datasets mentioned above, the help documentation will helpful to explore: 
...
<br>
<br>
The following are examples of an error being thrown, and execution being stopped in `detect_outliers`:

```{r test_outlier, eval=FALSE}

# Example 1: The given tissue does not exist in tissue_map$protein_tissue
detect_outliers("brainstem")
"The tissue brainstem is not found in tissue_map$protein_tissue."

# Example 2: Slightly misspelled tissue names, "lvg" instead of "liver"
detect_outliers("lvg")
"The tissue lvg is not found in tissue_map$protein_tissue."
```
<br>
<br>

**Demonstration of function `detect_outliers`:**
<br>
<br>
...
<br>
***Note:***
If you are interested in obtaining the top 10 up-regulated genes, top 10 down-regulated genes or top 10 no change genes from the cancer metadata tibble, then you can filter the output$table to obtain these lists.<br>
...
`compareCancerProtein` performs checks, to prevent the user from providing the wrong input type.The correct input type for `compareCancerProtein` is a character string specifying the cancer type of interest.The value must be one of the valid cancer names available in {cancer_tissue_map$cancer}. If an invalid cancer type is provided, the function will stop execution and throw an error.
...
# Explore the valid cancer types
...
The following are examples of an error being thrown, and execution being stopped in `compareCancerProtein`:

```{r test_cancer, eval=FALSE}

# Example 1: The given tissue does not exist in cancer_tissue_map$cancer
compareCancerProtein("bladder cancer")
"The cancer type bladder cancer is not found in cancer_tissue_map$cancer."

# Example 2: Slightly misspelled cancer name, "breastt cancer" instead of "breast cancer"
compareCancerProtein("breastt cancer")
"The cancer type breastt cancer is not found in cancer_tissue_map$cancer."

```
<br>
<br>

**Demonstration of function `compareCancerProtein`:**
<br>
<br>
...
```
```{r,echo=FALSE, fig.cap="Differential Expression in Protein for Breast Cancer"}
...
# Tutorial: Connecting the Parts of ExprCompareR
<br>
This section demonstrates how a user might combine the core functions of ExprCompareR in a single analysis workflow.
Let us walk through a simple use case:
<br>
<br>
**Use Case: Investigate whether the gene TSPAN6 may play a role in breast cancer.**
<br>
<br>
**1. Explore RNA–Protein Concordance with `compute_correlation()`**
<br>

We begin by examining how well RNA expression aligns with protein expression for `TSPAN6` in contrast to other important pan-genes in a standard set of tissues.

```{r TSPAN6_CC,eval=FALSE}
output<- compute_correlation(gene_NAMES = c("TSPAN6", "BRCA1", "TP53", "EGFR", "MYC"))
output$per_gene_plot

```
```{r,echo=FALSE, fig.cap="Spearman Correlation per gene across various genes"}
knitr::include_graphics("images/TSPAN6_correlation_gene.png")
```

<br>
**Interpretation of plot:** <br>

`TSPAN6` has a Spearman correlation of 0.06, the same as `TP53`. This can suggest some similar post-transcriptional regulation on both genes. Also, 0.06 is lower compared to `EGFR` (0.5), which can also suggest some differences in gene function.
<br>
<br>

**2. Compare RNA–Protein Correlation Across Genes and Tissues with `correlation_genes_tissues()`**
<br>
Next, we can examine how `TSPAN6` behaves across multiple tissues and compare it to other genes.

```{r TSPAN6_CGT,eval=FALSE}
result <- correlation_genes_tissues(
  gene_NAMES  = c("TSPAN6", "BRCA1", "TP53","EGFR", "MYC"),
  tissue_NAMES = c("breast", "lung", "liver", "colon", "kidney"),
  plot_choice = "per_gene"
)
result$per_gene_plot

```
```{r,echo=FALSE, fig.cap="Spearman Correlation per gene across various tissues of interest"}
knitr::include_graphics("images/TSPAN6_corr_genes_tissues.png")
```
<br>
**Interpretation of plot:** <br>

Across these set of tissues, we see that `TSPAN6` has a high Spearman correlation(0.45).
Notice that our set of tissues(tissue_NAMES) is different from the standard set of tissues(tissue_list_RNA).
<br>
<br>

**3. Detect Expression Outliers with `detect_outliers()`**
<br>

To see whether `TSPAN6` behaves unusually in breast tissue relative to other genes, we can identify outliers—
genes where RNA and protein levels differ substantially.
<br>

```{r TSPAN6_od, eval=FALSE}
output <- detect_outliers(input_tissue ="breast")
output$outlier_vector
 [1] "USH1C"     "VIM"       "STAP1"     "CAMK2A"    "ENO1"      "ACTB"      "CD59"      "HSP90AB1"  "HSPB1"    
[10] "KHDRBS2"   "TTK"       "PEX5L"     "EEF1B2"    "NT5C1A"    "DDC"       "CENPE"     "ZNF593"    "RPS27A"   
[19] "SLC4A10"   "MED12L"    "VWA5B2"    "GC"        "BEND6"     "SPHKAP"    "EEF1A1"    "KIF26B"    "S100A11"  
[28] "BSN"       "COL22A1"   "FABP4"     "JUN"       "CDCA2"     "WT1"       "TMPRSS11A" "SBK2"      "APOD"     
[37] "ADH1B"     "FABP12"    "PSAP"      "S100A6"    "RPS4X"     "MT-CO2"    "MT-CO1"    "MT-ND3"    "HLA-DRA"  
[46] "EIF3CL"    "XKR9"      "HLA-B"     "EEF1G"     "CAPNS2"    "RPL17"  

output$outlier_plot
```
```{r,echo=FALSE, fig.cap="Outliers detected in breast tissue"}
knitr::include_graphics("images/TSPAN6_outlier.png")
```
<br>
<br>
**Interpretation of plot and list:** <br>

We observe from the plot and list that `TSPAN6` is not labeled an outlier. This means that the difference in RNA vs protein expression of `TSPAN6` is not as high compared to other genes in the breast tissue. This can imply that maybe there are other genes that have a higher delta which maybe implicated in breast cancer.

<br>
<br>

**4. Compare Protein Expression Between Normal and Cancer Tissue with compareCancerProtein()**
<br>

Finally, we can investigate whether `TSPAN6` protein levels change between normal breast tissue and breast cancer.

```{r TSPAN6_cancer, eval=FALSE}
...
```
```{r,echo=FALSE, fig.cap="Differential Expression in Protein for Breast Cancer"}
knitr::include_graphics("images/TSPAN6_cancer.png")
```

```{r TSPAN6_cancer_table, eval=FALSE}
output$table
# A tibble: 5,378 × 6
   ensembl         gene     normal_rank cancer_rank delta_rank direction
   <chr>           <chr>          <dbl>       <dbl>      <dbl> <chr>    
...
# To filter for the TSPAN6 gene
library(dplyr)
output$table%>% filter(gene == "TSPAN6")
# A tibble: 1 × 6
  ensembl         gene   normal_rank cancer_rank delta_rank direction
  <chr>           <chr>        <dbl>       <dbl>      <dbl> <chr>    
1 ENSG00000000003 TSPAN6           1        1.75       0.75 Up  
  
# Filter for genes that are also up regulated and have the highest delta rank
  
output$table %>%
  filter(direction == "Up") %>%
  filter(delta_rank == max(delta_rank))
# A tibble: 10 × 6
   ensembl         gene     normal_rank cancer_rank delta_rank direction
   <chr>           <chr>          <dbl>       <dbl>      <dbl> <chr>    
 1 ENSG00000081853 PCDHGA2            1           3          2 Up       
 2 ENSG00000114529 C3orf52            1           3          2 Up       
 3 ENSG00000146776 ATXN7L1            1           3          2 Up       
 4 ENSG00000162642 C1orf52            1           3          2 Up       
 5 ENSG00000240184 PCDHGC3            1           3          2 Up       
 6 ENSG00000253159 PCDHGA12           1           3          2 Up       
 7 ENSG00000253485 PCDHGA5            1           3          2 Up       
 8 ENSG00000253731 PCDHGA6            1           3          2 Up       
 9 ENSG00000253873 PCDHGA11           1           3          2 Up       
10 ENSG00000254122 PCDHGB7            1           3          2 Up 


```

<br>
**Interpretation of table and plot:** <br>

We observe from the plot that most of the genes in breast cancer are up-regulated.
We also observe that `TSPAN6` is one of those up-regulated genes. It has a delta of 0.75 between its normal vs cancer expression. However, if we compare that difference to other up-regulated genes, we see there exist genes like `PCDHGC3` or `ATXN7L1` which have a delta rank of 2. This suggests that maybe along with `TSPAN6`, there are other genes implicated in breast cancer. 

<br>

Following these results with wet-lab experiments, these results from ExprCompareR may help the user conclude if `TSPAN6` has been mutated such that there are defects in the post transcriptional regulation or that `TSPAN6` is a possible therapeutic target for drugs involved in treatment of breast cancer.
